...
function saveUserInDatabase($con,$email,$username,$token,$provider){
    ...
    if (mysqli_num_rows($check_user) > 0) {
        ...
        $qr = mysqli_query($con, "INSERT INTO `tb_firebaseauthusers`( `username`, `email`, `token`, `created_at`, `login_type`) VALUES ('" . $username . "','" . $email . "','" . $token . "','" . date('Y-m-d H:i:s') . "','" . $provider . "')");
...
curl_setopt_array($curl, array(
    CURLOPT_URL => "https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=AIzaSyCzgwpNtavE1LOg927fYTmT6xx7tFrcJsI&idToken=" . $token,
