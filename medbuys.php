...
switch($_GET["action"]) {
	case "add":
		if(!empty($_POST["quantity"])) {
			$productByCode = $db_handle->runQuery("SELECT * FROM tb_medicinereg WHERE fkey='" . $_GET["code"] . "'");
	...
	<section id="about" style="background-color: #ecf5ff; box-shadow: 0px 0px 12px 0px #aeb8ba;">
		<div class="container ">
			...
			<header class="section-header">
				<h3>Add Medicines To Cart - Karunya Medicals</h3><br>
			</header>
...
<p align="right"><a id="btnEmpty" href="medbuys.php?action=empty"><button class="btn btn-info">Empty Cart</button></a></p>
<?php
...
<form action="placeorderregmed.php" method="POST">
...
<div id="product-grid">
	<div class="txt-heading"><br><header class="section-header">
				<h5 align="center">Available Medicines</h5><br>
			</header></div>
	...
	<table data-toggle="table" data-height="460" data-pagination="true" data-search="true" id="dataTable">
		...
		<tbody>
			...
			$curdate = date("l jS \of F Y");
				$product_array = $db_handle->runQuery("SELECT * FROM tb_medicinereg where loginid='".$_SESSION['kitkey']."' and fqty>'0' and fstatus='1' and delstatus!='1'");//echo $_SESSION['kitkey'];exit;
				if (!empty($product_array)) { 
					foreach($product_array as $key=>$value){ ?>
						<form method="post" action="medbuys.php?action=add&code=<?php echo $product_array[$key]["fkey"]; ?>">
							<tr>
